...
import { visitSchema } from "./visit_schema.ts";
...
export class SchemaDirectiveVisitor<TArgs = any, TContext = any>
  ...
  public static getDirectiveDeclaration(
    ...
    return (schema as any).getDirective(directiveName);
  }
  ...
  public static visitSchemaDirectives(
    ...
    const directiveVisitorMap: Record<string, typeof SchemaDirectiveVisitor> =
    ...
    function visitorSelector(
      ...
      if (directivesInExtensions != null) {
        ...
        let directiveNodes = (type as any)?.astNode?.directives ?? [];

        const extensionASTNodes:
          | ReadonlyArray<TypeSystemExtensionNode>
          | undefined = (type as {
  ...
  protected static getDeclaredDirectives(
    schema: typeof GraphQLSchema,
    directiveVisitors: Record<string, SchemaDirectiveVisitorClass>,
  ): Record<string, typeof GraphQLDirective> {
    const declaredDirectives: Record<string, typeof GraphQLDirective> =
      (schema as any).getDirectives().reduce(
        (prev: any, curr: any) => ({
          ...prev,
          [curr.name]: curr,
        }),
